...
const DEFAULT_SETTINGS: SiteSettings = {
  ...
  welcomeMsg: 'สวัสดีค่ะ! ฉันชื่อ โอลี่ 🌿',
    ...
    <div style={{ minHeight: '100vh', background: '#f3f4f6', fontFamily: 'Sarabun, sans-serif', color: '#1f2937' }}>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '24px 20px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16, marginBottom: 24 }}>
          {[
            { label: 'สมาชิกทั้งหมด', val: users.length, bg: 'linear-gradient(135deg,#0d7377,#14a085)', shadow: '#14a085' },
            { label: 'ฐานความรู้', val: kb.length, bg: 'linear-gradient(135deg,#d97706,#f59e0b)', shadow: '#f59e0b' },
            { label: 'โปรโมชั่น & ประชุม', val: schedule.length, bg: 'linear-gradient(135deg,#6d28d9,#8b5cf6)', shadow: '#8b5cf6' },
          ].map((s, i) => (
            <div key={i} style={{ background: s.bg, borderRadius: 16, padding: '22px 24px', color: '#fff', boxShadow: '0 4px 15px ' + s.shadow + '44' }}>
              <div style={{ fontSize: 38, fontWeight: 800, lineHeight: 1 }}>{s.val}</div>
              <div style={{ fontSize: 14, marginTop: 8, opacity: 0.9 }}>{s.label}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 20, flexWrap: 'wrap' }}>
          {([
            ...
            <button key={id} onClick={() => setTab(id as typeof tab)} style={{
              ...
              borderColor: tab === id ? GR : '#e5e7eb',
              background: tab === id ? GR : '#fff',
              color: tab === id ? '#fff' : '#6b7280',
              boxShadow: tab === id ? '0 2px 8px #14a08555' : 'none',
            }}>
              {label}
              {count !== null && (
          ...
          <div style={{ background: '#fff', borderRadius: 16, boxShadow: '0 1px 4px rgba(0,0,0,0.08)', overflow: 'hidden' }}>
            ...
            {filteredUsers.length === 0 ? (
              <div style={{ padding: '56px', textAlign: 'center', color: '#9ca3af' }}>
                ...
                {!search && <div style={{ fontSize: 13 }}>สมาชิกจะปรากฏที่นี่เมื่อลงทะเบียนผ่านหน้าบอท</div>}
              </div>
            ) : (
            ...
            {showAddKB && (
              <div style={{ padding: '20px 24px', background: '#f9fafb', borderBottom: '1px solid #f3f4f6' }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <button onClick={addKB} disabled={!kbTitle.trim() || !kbContent.trim() || saving} style={{ padding: '10px 24px', borderRadius: 20, border: 'none', background: kbTitle.trim() && kbContent.trim() ? GR : '#d1d5db', color: '#fff', fontFamily: 'inherit', fontSize: 14, fontWeight: 600, cursor: kbTitle.trim() ? 'pointer' : 'not-allowed' }}>
                    {saving ? '⏳' : '💾 บันทึก'}
                  </button>
        ...
        {tab === 'settings' && (
          <div style={{ background: '#fff', borderRadius: 16, boxShadow: '0 1px 4px rgba(0,0,0,0.08)', overflow: 'hidden' }}>
            <div style={{ padding: '20px 24px', borderBottom: '1px solid #f3f4f6' }}>
              ...
              <div style={{ color: '#6b7280', fontSize: 13, marginTop: 4 }}>ข้อมูลนี้จะแสดงในหน้า Member Page /m?ref=CODE</div>
            </div>
            <div style={{ padding: '24px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
              <div>
                <div style={{ fontSize: 13, fontWeight: 600, color: GR, marginBottom: 6 }}>ชื่อแบรนด์</div>
              ...
              <div>
                <div style={{ fontSize: 13, fontWeight: 600, color: GR, marginBottom: 6 }}>ลิงก์ Facebook</div>
              ...
              <div style={{ gridColumn: '1/-1' }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: GR, marginBottom: 6 }}>คำโปรย หลัก (Hero Headline)</div>
              ...
              <div style={{ gridColumn: '1/-1' }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: GR, marginBottom: 6 }}>คำโปรย รอง</div>
              ...
              <div style={{ gridColumn: '1/-1' }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: GR, marginBottom: 6 }}>ข้อความต้อนรับของบอท</div>
              ...
              <div>
                <div style={{ fontSize: 13, fontWeight: 600, color: GR, marginBottom: 6 }}>เบอร์โทรศัพท์</div>
                <input style={ins} value={settings.phone} onChange={e => setSettings(p => ({ ...p, phone: e.target.value }))} placeholder="08x-xxx-xxxx" />
              </div>
              <div>
                <div style={{ fontSize: 13, fontWeight: 600, color: GR, marginBottom: 6 }}>ลิงก์ LINE</div>
              ...
              <div style={{ gridColumn: '1/-1' }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: GR, marginBottom: 6 }}>แผนรายได้ (แต่ละบรรทัด = 1 ข้อ)</div>
              ...
              <div style={{ gridColumn: '1/-1' }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: GR, marginBottom: 12 }}>ภาพโปสเตอร์แผนรายได้ (URL สูงสุด 3 ภาพ)</div>
