...
interface UserData {
  ...
  facebookUrl?: string; profileImg?: string; refBaseUrl?: string;
  heroTitle?: string; createdByAdmin?: boolean; ts: number;
}
interface KBEntry { id: string; title: string; content: string; ts: number; }
interface ScheduleItem {
  id: string; type: string; week: number; day: number; category: string;
  title: string; desc: string; startTime: string; endTime: string; location?: string;
}
...
interface MemberForm {
  username: string; slug: string; phone: string; lineUrl: string;
  messengerUrl: string; facebookUrl: string; profileImg: string;
  refBaseUrl: string; heroTitle: string;
}

const BLANK: MemberForm = { username:'', slug:'', phone:'', lineUrl:'', messengerUrl:'', facebookUrl:'', profileImg:'', refBaseUrl:'', heroTitle:'' };
...
export default function AdminPage() {
  ...
  const [memberForm, setMemberForm] = useState<MemberForm>(BLANK);
  const [memberSaving, setMemberSaving] = useState(false);
  const [editingUser, setEditingUser] = useState<UserData | null>(null);
  const [editForm, setEditForm] = useState<MemberForm>(BLANK);
  ...
  const poster1Ref = useRef<HTMLInputElement>(null);
  const poster2Ref = useRef<HTMLInputElement>(null);
  const poster3Ref = useRef<HTMLInputElement>(null);
  const posterRefs: Record<string, React.RefObject<HTMLInputElement>> = {
    poster1: poster1Ref, poster2: poster2Ref, poster3: poster3Ref,
  };

  const notify = (m: string) => { setMsg(m); setTimeout(() => setMsg(''), 3000); };
  const schUp = (k: string) => (v: string | number) => setSchForm(p => ({ ...p, [k]: v }));
  const mUp = (k: keyof MemberForm) => (v: string) => setMemberForm(p => ({ ...p, [k]: v }));
  const eUp = (k: keyof MemberForm) => (v: string) => setEditForm(p => ({ ...p, [k]: v }));
  ...
  useEffect(() => {
    ...
    const s = memberForm.username.toLowerCase().replace(/\s+/g, '').replace(/[^a-z0-9\u0E00-\u0E7F]/g, '').slice(0, 10);
    setMemberForm(p => ({ ...p, slug: s }));
  }, [memberForm.username]);
  ...
  async function handleProfileUpload(file: File, forEdit = false) {
    setUploadingProfile(true);
    const url = await uploadImg(file);
    if (url) {
      if (forEdit) setEditForm(p => ({ ...p, profileImg: url }));
      else setMemberForm(p => ({ ...p, profileImg: url }));
      notify('อัปโหลดรูปแล้ว!');
    } else notify('อัปโหลดไม่สำเร็จ — ลองวาง URL Imgur แทนค่ะ');
    setUploadingProfile(false);
  }

  async function handlePosterUpload(key: string, file: File) {
    setUploadingPoster(key);
    const url = await uploadImg(file);
    if (url) { setSettings(p => ({ ...p, [key]: url })); notify('อัปโหลดรูปแล้ว!'); }
    else notify('อัปโหลดไม่สำเร็จ — ลองวาง URL แทนค่ะ');
    setUploadingPoster('');
  }
  ...
  async function delSch(id: string) {
    if (!confirm('ลบ?')) return;
  ...
  async function addMember() {
    ...
    const u = await api('/api/users', 'POST', {
      ...
      phone: memberForm.phone, slug: memberForm.slug || undefined,
      lineUrl: memberForm.lineUrl, messengerUrl: memberForm.messengerUrl,
      facebookUrl: memberForm.facebookUrl, profileImg: memberForm.profileImg,
      refBaseUrl: memberForm.refBaseUrl, heroTitle: memberForm.heroTitle,
      createdByAdmin: true,
    });
    if (!u.error) { setUsers(p => [u, ...p]); setMemberForm(BLANK); setShowAddMember(false); notify('เพิ่มสมาชิกแล้ว!'); }
    else notify(u.error);
  ...
  function openEdit(u: UserData) {
    setEditingUser(u);
    setEditForm({
      username: u.username, slug: u.refCode,
      phone: u.phone || '', lineUrl: u.lineUrl || '',
      messengerUrl: u.messengerUrl || '', facebookUrl: u.facebookUrl || '',
      profileImg: u.profileImg || '', refBaseUrl: u.refBaseUrl || '',
  ...
  async function saveEdit() {
    ...
    const updated = await api('/api/users', 'PUT', {
      ...
      phone: editForm.phone, lineUrl: editForm.lineUrl,
      messengerUrl: editForm.messengerUrl, facebookUrl: editForm.facebookUrl,
      profileImg: editForm.profileImg, refBaseUrl: editForm.refBaseUrl,
      heroTitle: editForm.heroTitle,
    });
    if (!updated?.error) {
      setUsers(p => p.map(u => u.id === editingUser.id ? { ...u, ...updated } : u));
      setEditingUser(null); notify('แก้ไขข้อมูลแล้ว!');
  ...
  async function saveSettings() {
    ...
    notify('บันทึกแล้ว!'); setSaving(false);
  }
  ...
  const TABS = [
    { id: 'members', label: '👥 สมาชิก', count: users.length },
    { id: 'kb', label: '📝 ฐานความรู้', count: kb.length },
    { id: 'schedule', label: '📅 ตาราง/โปร', count: schedule.length },
    { id: 'settings', label: '⚙️ ตั้งค่าบอท', count: -1 },
  ];

  function MemberModalBody({ form, setForm, onSave, onClose, saving: isSaving, title, btnColor }: {
    form: MemberForm; setForm: (f: MemberForm) => void;
    onSave: () => void; onClose: () => void;
    saving: boolean; title: string; btnColor: string;
  }) {
    ...
    return (
      <div style={{ position:'fixed',inset:0,zIndex:1000,background:'rgba(0,0,0,0.5)',backdropFilter:'blur(4px)',display:'flex',alignItems:'center',justifyContent:'center',padding:20 }}
        ...
        <div style={{ background:'#fff',borderRadius:20,width:'min(95vw,600px)',maxHeight:'92vh',overflowY:'auto',boxShadow:'0 32px 80px rgba(0,0,0,0.3)' }}>
          <div style={{ padding:'20px 24px',borderBottom:'1px solid #f3f4f6',display:'flex',alignItems:'center',justifyContent:'space-between',position:'sticky',top:0,background:'#fff',zIndex:1 }}>
            ...
            <button onClick={onClose} style={{ width:32,height:32,borderRadius:'50%',border:'1.5px solid #e5e7eb',background:'#fff',color:'#6b7280',fontSize:18,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center' }}>×</button>
          </div>
          <div style={{ padding:'22px 24px' }}>
            <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:14,marginBottom:16 }}>
              <div>
                <div style={{ fontSize:13,fontWeight:600,color:GR,marginBottom:6 }}>ชื่อที่แสดง *</div>
                <input style={ins} value={form.username} onChange={e => setForm({ ...form, username: e.target.value })} placeholder="เช่น โค้ชเพชร" />
              </div>
              <div>
                <div style={{ fontSize:13,fontWeight:600,color:GR,marginBottom:6 }}>slug (รหัส Ref){isEdit ? ' — อ่านอย่างเดียว' : ' *'}</div>
                {isEdit
                  ? <input style={{ ...ins, background:'#f9fafb', color:'#9ca3af' }} value={editingUser?.refCode || ''} readOnly />
                  : <input style={ins} value={form.slug} onChange={e => setForm({ ...form, slug: e.target.value.toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, 10) })} placeholder="เช่น PETCH" />}
                <div style={{ fontSize:11,color:'#9ca3af',marginTop:4 }}>ลิงก์: /m?ref={isEdit ? editingUser?.refCode : form.slug || 'CODE'}</div>
              </div>
            </div>
            <div style={{ marginBottom:14 }}>
              <div style={{ fontSize:13,fontWeight:600,color:GR,marginBottom:6 }}>ลิงก์สมัครส่วนตัว OlyLife</div>
              <input style={ins} value={form.refBaseUrl} onChange={e => setForm({ ...form, refBaseUrl: e.target.value })} placeholder="https://olylifeint.com/register?ref=..." />
            </div>
            <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:14,marginBottom:14 }}>
              <div>
                <div style={{ fontSize:13,fontWeight:600,color:GR,marginBottom:6 }}>เบอร์โทร</div>
                <input style={ins} value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value })} placeholder="0812345678" />
              </div>
              <div>
                <div style={{ fontSize:13,fontWeight:600,color:GR,marginBottom:6 }}>ลิงก์ไลน์ (LINE)</div>
                <input style={ins} value={form.lineUrl} onChange={e => setForm({ ...form, lineUrl: e.target.value })} placeholder="https://line.me/ti/p/~id" />
              </div>
            </div>
            <div style={{ marginBottom:14 }}>
              <div style={{ fontSize:13,fontWeight:600,color:GR,marginBottom:6 }}>ลิงก์ Messenger</div>
              <input style={ins} value={form.messengerUrl} onChange={e => setForm({ ...form, messengerUrl: e.target.value })} placeholder="https://m.me/username" />
            </div>
            <div style={{ marginBottom:14 }}>
              <div style={{ fontSize:13,fontWeight:600,color:GR,marginBottom:6 }}>📘 Facebook ส่วนตัว</div>
              <input style={ins} value={form.facebookUrl} onChange={e => setForm({ ...form, facebookUrl: e.target.value })} placeholder="https://facebook.com/yourname" />
            </div>
            <div style={{ marginBottom:14 }}>
              ...
              <div style={{ display:'flex',gap:14,alignItems:'center' }}>
                <div style={{ width:72,height:72,borderRadius:'50%',border:'2px dashed #d1d5db',background:'#f9fafb',display:'flex',alignItems:'center',justifyContent:'center',overflow:'hidden',flexShrink:0,position:'relative' }}>
                  {form.profileImg
                    ? <img src={form.profileImg} alt="" style={{ width:'100%',height:'100%',objectFit:'cover' }} onError={e => { (e.target as HTMLImageElement).style.display = 'none'; }} />
                ...
                <div style={{ flex:1 }}>
                  <input type="file" accept="image/*" ref={profileFileRef} style={{ display:'none' }}
                    onChange={e => { const f = e.target.files?.[0]; if (f) handleProfileUpload(f, isEdit); e.target.value = ''; }} />
                  <button onClick={() => profileFileRef.current?.click()} disabled={uploadingProfile}
                    style={{ padding:'8px 16px',borderRadius:20,border:'1.5px solid #bbf7d0',background:GLIGHT,color:'#16a34a',fontSize:13,cursor:'pointer',fontFamily:'inherit',fontWeight:600,marginBottom:8,display:'block' }}>
                    📤 {uploadingProfile ? 'กำลังอัปโหลด...' : 'อัปโหลดรูป'}
                  </button>
                  <input style={{ ...ins, fontSize:12 }} value={form.profileImg} onChange={e => setForm({ ...form, profileImg: e.target.value })} placeholder="หรือวาง URL รูป เช่น https://i.imgur.com/xxx.jpg" />
                  {form.profileImg && (
                    <button onClick={() => setForm({ ...form, profileImg: '' })} style={{ marginTop:5,padding:'3px 10px',borderRadius:8,border:'1.5px solid #fecaca',background:'#fef2f2',color:'#ef4444',fontSize:11,cursor:'pointer',fontFamily:'inherit' }}>🗑 ลบรูป</button>
                  )}
                </div>
              </div>
            </div>
            <div style={{ marginBottom:22 }}>
              ...
              <input style={ins} value={form.heroTitle} onChange={e => setForm({ ...form, heroTitle: e.target.value })} placeholder="เช่น สุขภาพดี *เริ่มที่นี่*" />
              <div style={{ fontSize:11,color:'#9ca3af',marginTop:4 }}>ครอบคำด้วย *...* ให้เป็นสีไฮไลต์</div>
            </div>
            <div style={{ display:'flex',gap:12,justifyContent:'flex-end' }}>
              <button onClick={onClose} style={{ padding:'12px 22px',borderRadius:20,border:'1.5px solid #e5e7eb',background:'#fff',color:'#6b7280',fontFamily:'inherit',fontSize:14,cursor:'pointer' }}>ยกเลิก</button>
              <button onClick={onSave} disabled={!form.username.trim() || isSaving}
                style={{ padding:'12px 28px',borderRadius:20,border:'none',background:form.username.trim() ? btnColor : '#d1d5db',color:'#fff',fontFamily:'inherit',fontSize:15,fontWeight:700,cursor:form.username.trim() ? 'pointer' : 'not-allowed' }}>
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#f3f4f6', fontFamily:'Sarabun, sans-serif', color:'#1f2937' }}>
        ...
        <div style={{ display:'flex',gap:8,marginBottom:20,flexWrap:'wrap' }}>
          {TABS.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)} style={{
              padding:'9px 20px',borderRadius:30,border:'1.5px solid',cursor:'pointer',fontFamily:'inherit',fontSize:14,fontWeight:600,
              borderColor: tab === t.id ? GR : '#e5e7eb',
              background: tab === t.id ? GR : '#fff',
              color: tab === t.id ? '#fff' : '#6b7280',
              boxShadow: tab === t.id ? '0 2px 8px #14a08555' : 'none',
            }}>
              {t.label}
              {t.count >= 0 && <span style={{ background:tab===t.id?'rgba(255,255,255,0.25)':'#f3f4f6',borderRadius:12,padding:'1px 8px',fontSize:12,marginLeft:6 }}>{t.count}</span>}
            </button>
          ))}
        </div>
          ...
          <div style={{ background:'#fff',borderRadius:16,boxShadow:'0 1px 4px rgba(0,0,0,0.08)',overflow:'hidden' }}>
            <div style={{ padding:'20px 24px',borderBottom:'1px solid #f3f4f6',display:'flex',alignItems:'center',justifyContent:'space-between',gap:12,flexWrap:'wrap' }}>
              ...
              <div style={{ display:'flex',gap:10 }}>
                ...
                <button onClick={() => setShowAddMember(true)} style={{ padding:'9px 20px',borderRadius:20,border:'none',background:GR,color:'#fff',fontSize:14,cursor:'pointer',fontFamily:'inherit',fontWeight:700,whiteSpace:'nowrap' }}>+ เพิ่มสมาชิก</button>
              </div>
            </div>
              ...
              <table style={{ width:'100%',borderCollapse:'collapse' }}>
                <thead>
                  <tr style={{ background:'#f9fafb' }}>
                    {['#','สมาชิก','เบอร์/Social','รหัส Ref','แนะนำโดย','วันที่','ลิงก์','จัดการ'].map(h => (
                  ...
                  {filteredUsers.map((u, i) => (
                    <tr key={u.id} style={{ borderBottom:'1px solid #f9fafb',transition:'background .1s' }}
                      ...
                      <td style={{ padding:'12px 16px' }}>
                        <div style={{ display:'flex',alignItems:'center',gap:10 }}>
                          {u.profileImg
                            ? <img src={u.profileImg} alt="" style={{ width:38,height:38,borderRadius:'50%',objectFit:'cover',border:'2px solid #d1fae5' }} onError={e => { (e.target as HTMLImageElement).style.display='none'; }} />
                            : <div style={{ width:38,height:38,borderRadius:'50%',background:'linear-gradient(135deg,#0d7377,#14a085)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:16,color:'#fff',fontWeight:700,flexShrink:0 }}>{u.username.charAt(0).toUpperCase()}</div>}
                          <div>
                            <div style={{ fontWeight:600,fontSize:14 }}>{u.username}</div>
                            {u.createdByAdmin && <div style={{ fontSize:11,color:GR }}>✦ Admin</div>}
                          </div>
                        </div>
                      </td>
                      <td style={{ padding:'12px 16px',fontSize:12,color:'#6b7280' }}>
                        {u.phone && <div>📞 {u.phone}</div>}
                        {u.lineUrl && <a href={u.lineUrl} target="_blank" rel="noreferrer" style={{ color:GR,display:'block' }}>💬 LINE</a>}
                        {u.facebookUrl && <a href={u.facebookUrl} target="_blank" rel="noreferrer" style={{ color:'#3b82f6',display:'block' }}>📘 FB</a>}
                      </td>
                      <td style={{ padding:'12px 16px' }}>
          ...
          <div style={{ background:'#fff',borderRadius:16,boxShadow:'0 1px 4px rgba(0,0,0,0.08)',overflow:'hidden' }}>
            ...
            <table style={{ width:'100%',borderCollapse:'collapse' }}>
              ...
              <tbody>
                ...
                {kb.map((e,i) => (
                  <tr key={e.id} onMouseEnter={ev=>(ev.currentTarget.style.background='#f9fafb')} onMouseLeave={ev=>(ev.currentTarget.style.background='#fff')} style={{ borderBottom:'1px solid #f9fafb' }}>
          ...
          <div style={{ background:'#fff',borderRadius:16,boxShadow:'0 1px 4px rgba(0,0,0,0.08)',overflow:'hidden' }}>
            ...
            {showAddSch && (
              <div style={{ padding:'20px 24px',background:'#f9fafb',borderBottom:'1px solid #f3f4f6' }}>
                <div style={{ display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:12,marginBottom:12 }}>
                  ...
                  <div><div style={{ fontSize:13,fontWeight:600,color:'#374151',marginBottom:6 }}>{schForm.type==='promo'?'รายละเอียด':'สถานที่'}</div><input style={ins} value={schForm.type==='promo'?schForm.desc:schForm.location} onChange={e=>schUp(schForm.type==='promo'?'desc':'location')(e.target.value)} placeholder={schForm.type==='promo'?'เงื่อนไข...':'Zoom link...'} /></div>
            ...
            <table style={{ width:'100%',borderCollapse:'collapse' }}>
              ...
              <tbody>
                ...
                {schedule.map((s,i) => (
                  <tr key={s.id} onMouseEnter={ev=>(ev.currentTarget.style.background='#f9fafb')} onMouseLeave={ev=>(ev.currentTarget.style.background='#fff')} style={{ borderBottom:'1px solid #f9fafb' }}>
                    <td style={{ padding:'14px 16px',color:'#9ca3af',fontSize:13 }}>{i+1}</td>
        ...
        {tab === 'settings' && (
          <div style={{ background:'#fff',borderRadius:16,boxShadow:'0 1px 4px rgba(0,0,0,0.08)',overflow:'hidden' }}>
            ...
            <div style={{ padding:'24px',display:'grid',gridTemplateColumns:'1fr 1fr',gap:20 }}>
              ...
              <div><div style={{ fontSize:13,fontWeight:600,color:GR,marginBottom:6 }}>ลิงก์ Facebook หลัก</div><input style={ins} value={settings.facebookUrl} onChange={e => setSettings(p => ({ ...p, facebookUrl: e.target.value }))} /></div>
              ...
              <div><div style={{ fontSize:13,fontWeight:600,color:GR,marginBottom:6 }}>ลิงก์ LINE</div><input style={ins} value={settings.lineUrl} onChange={e => setSettings(p => ({ ...p, lineUrl: e.target.value }))} /></div>
              <div><div style={{ fontSize:13,fontWeight:600,color:GR,marginBottom:6 }}>ลิงก์ Messenger</div><input style={ins} value={settings.messengerUrl} onChange={e => setSettings(p => ({ ...p, messengerUrl: e.target.value }))} /></div>
                  ...
                  <div key={k} style={{ marginBottom:16 }}>
                    ...
                    <div style={{ display:'flex',gap:12,alignItems:'flex-start' }}>
                      <div style={{ width:90,height:90,borderRadius:12,border:'2px dashed #d1d5db',background:settings[k]?'transparent':'#f9fafb',display:'flex',alignItems:'center',justifyContent:'center',overflow:'hidden',flexShrink:0,position:'relative' }}>
                        {settings[k]?<img src={settings[k]} alt="" style={{ width:'100%',height:'100%',objectFit:'cover' }} onError={e=>{(e.target as HTMLImageElement).style.display='none';}} />:<span style={{ fontSize:28,opacity:.3 }}>🖼️</span>}
                      ...
                      <div style={{ flex:1 }}>
                        <div style={{ display:'flex',gap:8,marginBottom:8 }}>
                          ...
                          {settings[k]&&<button onClick={() => setSettings(p => ({ ...p, [k]: '' }))} style={{ padding:'7px 16px',borderRadius:20,border:'1.5px solid #fecaca',background:'#fef2f2',color:'#ef4444',fontSize:13,cursor:'pointer',fontFamily:'inherit',fontWeight:600 }}>🗑 ลบรูป</button>}
                        </div>
                        <input style={{ ...ins, fontSize:12 }} value={settings[k]} onChange={e => setSettings(p => ({ ...p, [k]: e.target.value }))} placeholder="หรือวาง URL ภาพ เช่น https://i.imgur.com/xxx.jpg" />
                      </div>
                    </div>
                  </div>
                ))}
      ...
      {showAddMember && (
        <MemberModalBody
          form={memberForm} setForm={setMemberForm}
          onSave={addMember} onClose={() => setShowAddMember(false)}
          saving={memberSaving} title="เพิ่มสมาชิกใหม่"
          btnColor="linear-gradient(135deg,#0d7377,#14a085)"
        />
      )}
      ...
      {editingUser && (
        <MemberModalBody
          ...
          onSave={saveEdit} onClose={() => setEditingUser(null)}
          saving={editSaving} title="✏️ แก้ไขข้อมูลสมาชิก"
          btnColor="linear-gradient(135deg,#2563eb,#3b82f6)"
        />
      )}
